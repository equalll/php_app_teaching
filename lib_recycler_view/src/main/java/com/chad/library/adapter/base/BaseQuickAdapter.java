...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    private View mNetworkErrorEmptyView;//网络异常导致 RecyclerView 空的占位布局
    private TextView tvErrorMsg;//网络异常.链接超时/服务器返回数据格式错误 导致 ；提示语
    private onClickEmptyViewListener onClickEmptyViewListener;//数据为空导致 RecyclerView 空的占位布局
    ...
    private void setRecyclerView(RecyclerView recyclerView) {
        ...
        //监听 mRecyclerView 滑动, 添加上拉自动加载机制
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if( dy > 0){
                    ...
                    autoLoadMore(manager.findLastCompletelyVisibleItemPosition());
                }
            }
        });
    }
     ...
     *
     */
    public void setNoDataEmptyView(View mNoDataEmptyView) {
        this.mNoDataEmptyView = mNoDataEmptyView;
    }
     ...
     *
     */
    public void setNetworkErrorEmptyView(View mNetworkErrorEmptyView,TextView textView) {
        this.mNetworkErrorEmptyView = mNetworkErrorEmptyView;
        this.tvErrorMsg = textView;
        if( mNetworkErrorEmptyView == null ){
            return;
        }
        mNetworkErrorEmptyView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if( onClickEmptyViewListener == null | !mLoadMoreEnable){
                    return;
                }
                onClickEmptyViewListener.onClickNetErrorEmptyView();
            }
        });
    }
    ...
    public void setEmptyViewEnable(boolean enable){
        if( mNoDataEmptyView != null ){
            mNoDataEmptyView.setEnabled(enable);
        }
        if( mNetworkErrorEmptyView != null){
            mNetworkErrorEmptyView.setEnabled(enable);
        }
    }

    public void setOnClickEmptyViewListener(BaseQuickAdapter.onClickEmptyViewListener onClickEmptyViewListener) {
...
//        autoLoadMore(position);//原版本。 如果放在这里的话当数据不满一页时，会不停的加载更多
     ...
     * 显示网络异常 占位布局
     */
    public void showNetworkErrorEmptyView(String msg){
        if(mNetworkErrorEmptyView == null){
            return;
        }
        if(tvErrorMsg != null){
        ...
        removeAllFooterView();
        setEmptyView(mNetworkErrorEmptyView);
    }
    ...
    private int previousPosition;//上次进来的 position 。
